...
public class ConditionalFieldAttributeDrawer : PropertyDrawer {
	...
	private void Initialize(SerializedProperty property) {
		...
		if (!_conditionalToTarget.ContainsKey(property))
			_conditionalToTarget.Add(property,
					ConditionalFieldUtility.FindRelativeProperty(property, _attribute.FieldToCheck));
		...
		} else if (!typeof(int).Module.ScopeName.Equals(fieldInfo?.FieldType.Module.ScopeName)) {
			_specialType = true;
	...
	public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
		...
		_toShow = ConditionalFieldUtility.PropertyIsVisible(_conditionalToTarget[property],
				_attribute.Inverse,
				_attribute.CompareValues);
		if (!_toShow) return 0;

		if (_genericDrawerInstance != null) return _genericDrawerInstance.GetPropertyHeight(property, label);
	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
		...
		if (_multipleAttributes && _genericDrawerInstance != null) {
			try { _genericDrawerInstance.OnGUI(position, property, label); } catch (Exception e) {
			...
			try { _genericTypeDrawerInstance.OnGUI(position, property, label); } catch (Exception e) {
				EditorGUI.PropertyField(position, property, label);
	...
	private void GetPropertyDrawerType(SerializedProperty property) {
		...
		try {
			_genericDrawerType = _typesCache.First(x =>
					(Type) CustomAttributeData.GetCustomAttributes(x).First().ConstructorArguments.First().Value ==
					_genericAttribute.GetType());
		} catch (Exception e) {
			LogWarning("Can't find property drawer from CustomPropertyAttribute of " +
					_genericAttribute.GetType() +
					" : " +
					e,
					property);
		...
		try {
			...
			foreach (CustomAttributeTypedArgument singleParam in attributeParams) {
				if (singleParam.Value.GetType() == typeof(ReadOnlyCollection<CustomAttributeTypedArgument>)) {
					foreach (CustomAttributeTypedArgument unpackedSingleParam in
							(ReadOnlyCollection<CustomAttributeTypedArgument>) singleParam.Value) {
						unpackedParams.Add(unpackedSingleParam);
					}
				} else { unpackedParams.Add(singleParam); }
			}
			...
			if (attributeParamsObj.Any()) {
				_genericAttribute =
						(PropertyAttribute) Activator.CreateInstance(_genericAttribute.GetType(), attributeParamsObj);
			} else { _genericAttribute = (PropertyAttribute) Activator.CreateInstance(_genericAttribute.GetType()); }
		} catch (Exception e) {
...
#region Find Relative Property
	public static SerializedProperty FindRelativeProperty(SerializedProperty property, string propertyName) {
		...
		if (nestedProperty == null) {
			...
			var who = "Property <color=brown>" +
					arrayProp.name +
					"</color> in object <color=brown>" +
					target.name +
					"</color> caused: ";
	...
	private static SerializedProperty NestedPropertyOrigin(SerializedProperty property, string[] elements) {
		...
		for (int i = 0; i < elements.Length - 1; i++) {
			...
			parent = i == 0 ? property.serializedObject.FindProperty(element) :
					parent != null ? parent.FindPropertyRelative(element) : null;
...
#region Behaviour Property Is Visible
	public static bool BehaviourPropertyIsVisible(MonoBehaviour behaviour,
			string propertyName,
			ConditionalFieldAttribute appliedAttribute) {
		...
		return ConditionalFieldUtility.PropertyIsVisible(targetProperty,
				appliedAttribute.Inverse,
				appliedAttribute.CompareValues);
	}
#endregion
}
#endif
